...
namespace NeuralNetworkSample.WPF.Models
{
    public class Coordinate
    {
        ...
        public static Coordinate operator +(Coordinate coordinate1, Coordinate coordinate2)
            => new() { Latitude = coordinate1.Latitude + coordinate2.Latitude, Longitude = coordinate1.Longitude + coordinate2.Longitude, CorrectValue = coordinate1.CorrectValue };
    ...
    class MathModel
    {
        public static List<DataPoint> SigmoidFunction => EnumerableExtension.Range(-8.0, 8.0, 0.1).Select<double, DataPoint>(x => new DataPoint(x, NeuralNetwork.Core.Math.Sigmoid(x))).ToList();
        public static List<DataPoint> DifferentialSigmoidFunction => EnumerableExtension.Range(-8.0, 8.0, 0.1).Select<double, DataPoint>(x => new DataPoint(x, NeuralNetwork.Core.Math.DifferentialSigmoid(x))).ToList();
    }
    ...
    abstract class DataModelBase
    {
        ...
        protected Coordinate baseCoordinate = new() { Latitude = 34.5, Longitude = 137.5 };

        protected IEnumerable<Coordinate> OriginalCoordinates { get; private set; } = [];

        public DataModelBase() => Load();

        protected static DataPoint ToDataPoint(Coordinate coordinate) => new(coordinate.Longitude, coordinate.Latitude);
    ...
    {
        public List<DataPoint> FukuiCoordinates { get; } = [];
        public List<DataPoint> OthersCoordinates { get; } = [];

        readonly NeuralNetwork.Core.NeuralNetwork neuralNetwork = new();
    ...
    {
        public List<DataPoint> LearningFukuiCoordinates { get; } = [];
        public List<DataPoint> LearningOthersCoordinates { get; } = [];
    ...
    class MachineLearningModel : SampleDataModel
    {
        ...
        public List<DataPoint> TrainingFukuiCoordinates { get; } = [];
        public List<DataPoint> TrainingOthersCoordinates { get; } = [];
        public List<DataPoint> TestFukuiCoordinates { get; } = [];
        public List<DataPoint> TestOthersCoordinates { get; } = [];
